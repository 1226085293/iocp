...
#include "network/websocket/public/server_base.h"
#include "network/websocket/iocp/server/client.h"
...
namespace network::websocket::iocp {
	// IOCP_TCP服务端
	class server final : public server_base, std::enable_shared_from_this<server> {
		// 共享资源临界区
	...
	public:
		...
		// 强制关闭客户端连接
		void forced_close_client(server_use::event_info* info_);
		// 关闭客户端连接
		void close_client(server_use::event_info* info_);
	};
